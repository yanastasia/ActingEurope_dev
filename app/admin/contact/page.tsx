...
export default function AdminContactPage() {
  const [contactPages, setContactPages] = useState<ContactPage[]>([]);
  const [isAdmin, setIsAdmin] = useState(false);
  const [editingPage, setEditingPage] = useState<ContactPage | null>(null);
  ...
  const handleCreatePage = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Contact page created successfully',
        });
        setNewPageData({
          title: '',
          content: '',
      ...
      toast({
        title: 'Error',
        description: 'Failed to create contact page',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDeletePage = async (id: number) => {
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Contact page deleted successfully',
        });
        fetchContactPages();
      } else {
      ...
      toast({
        title: 'Error',
        description: 'Failed to delete contact page',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Contact Page Management</h1>
      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button onClick={handleCreatePage}>Create Contact Page</Button>
            <Button
              variant="outline"
      ...
      {editingPage && (
        <Card className="mb-8">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleUpdatePage}>Update Contact Page</Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Existing Contact Pages</CardTitle>
        </CardHeader>
        <CardContent>
          {contactPages.length === 0 ? (
            <p className="text-center text-gray-500">No contact pages found.</p>
          ) : (
